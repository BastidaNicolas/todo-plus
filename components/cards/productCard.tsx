...
export default function ProductCard({onClick, code, selected}:{onClick:any, code:string|number, selected:string|number}) {
    ...
    return (
        <div className={`p-3 rounded-lg bg-white cursor-pointer drop-shadow-sm transition-all duration-200 border hover:border-black hover:drop-shadow-md ${selected === code && "border-black"}`} onClick={() => onClick(code)}>
